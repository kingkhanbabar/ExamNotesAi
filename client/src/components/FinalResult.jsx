...
const markDownComponent = {
  ...
  h2: ({ children }) => (
    <h1 className="text-xl font-semibold text-indigo-600 mt-5 mb-3">
      {children}
    </h1>
  ),
  h3: ({ children }) => (
    <h1 className="text-lg font-semibold text-gray-800 mt-4 mb-2">
      {children}
    </h1>
  ),
  p: ({ children }) => (
  ...
  ul: ({ children }) => (
    <p className="list-disc ml-6 space-y-1 text-gray-700">
      {children}
    </p>
  ),
  li: ({ children }) => (
    ...
    <div className="mt-6 p-3 space-y-10 bg-white">
      ...
      {!quickRevision && <section>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          <ReactMarkDowm components={markDownComponent}>
            {result.notes}
          </ReactMarkDowm>
      ...
      {quickRevision && <section className="rounded-xl bg-gradient-to-r from-green-100 to-green-50 border
     ...
     {result.diagram?.data && <section>
      <SectionHeader icon="📊" title="Diagram" volor="cyan"/>
      <MermaidSetup diagram={result.diagram.data}/>
      <p className="mt-3 text-xs text-gray-500 italic">
        ⬇️ If you need this diagram for future ref or revision,
      ...
      <section>
        ...
        <p className="mt-3 text-xs text-gray-500 italic">
        ⬇️ If you need this Chart for future ref or revision,
        you can save it by taking a screenshot
...
function SectionHeader({ icon, title, color }) {
  const colors = {
    ...
    purple: "from-rose-100 to-rose-50 text-rose-700"
  };
  return (
    <div className={`
